# Task 06 — CLI-Flags: --dockerfile und --meta

## Kontext

Projekt: copybirds (Go, stdlib only)
Arbeitsverzeichnis: Repo-Root

Tasks 01–05 haben alle Infrastruktur implementiert. Jetzt verdrahten wir die
neuen Optionen in den CLI-Kommandos.

## Dateien

- Ersetzen: `cmd/cb/cmd/docker.go`
- Modifizieren: `cmd/cb/cmd/run.go`

## Schritt 1 — docker.go komplett ersetzen

Ersetze den Inhalt von `cmd/cb/cmd/docker.go`:

```go
package cmd

import (
	"fmt"

	"github.com/spf13/cobra"

	"copybirds/internal/docker"
	"copybirds/internal/log"
)

func newDockerCmd() *cobra.Command {
	var baseImage, tag, customDockerfile, metaFile string

	cmd := &cobra.Command{
		Use:   "docker STAGING_DIR ENTRYPOINT_BINARY",
		Short: "Build Docker image from staging directory",
		Long: `Builds a Docker image from a staging directory (created by 'cb files').

When --meta is provided, cb reads the system metadata and:
  - Detects the correct Debian base image from the source distro
  - Installs packaged libraries via apt-get (standard repo + snapshot.debian.org)
  - Copies only non-packaged files from staging

When --dockerfile is provided, the custom Dockerfile is used as-is and
non-packaged files are injected via a COPY instruction at the end.`,
		Example: `  cb docker /tmp/staging usr/bin/myapp
  cb docker --meta myapp_meta.xml /tmp/staging usr/bin/myapp
  cb docker --dockerfile my.Dockerfile /tmp/staging usr/bin/myapp`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			stagingDir := args[0]
			entrypoint := args[1]

			opts := docker.DefaultOptions()
			opts.BaseImage = baseImage
			opts.Tag = tag
			opts.StagingDir = stagingDir
			opts.Entrypoint = entrypoint
			opts.AlpineWarning = true
			opts.CustomDockerfile = customDockerfile
			opts.MetaFile = metaFile

			log.Infof("cb_docker: Staging=%s Entrypoint=%s Base=%s Tag=%s",
				opts.StagingDir, opts.Entrypoint, opts.BaseImage, opts.Tag)

			if err := docker.Build(opts); err != nil {
				return fmt.Errorf("docker: %w", err)
			}

			log.Warnf("cb_docker: Docker image '%s' erfolgreich erstellt", opts.Tag)
			return nil
		},
	}

	cmd.Flags().StringVar(&baseImage, "base", "debian:bookworm-slim", "base Docker image (wird durch --meta überschrieben)")
	cmd.Flags().StringVar(&tag, "tag", "cb_app", "image tag")
	cmd.Flags().StringVar(&customDockerfile, "dockerfile", "", "eigenes Dockerfile (non-packaged files werden automatisch injiziert)")
	cmd.Flags().StringVar(&metaFile, "meta", "", "Pfad zur meta.xml für apt-basiertes Package-Management")

	return cmd
}
```

## Schritt 2 — run.go: --dockerfile Flag und CollectFilePackages

Öffne `cmd/cb/cmd/run.go` und nimm folgende Änderungen vor:

### 2a — Flag-Variable hinzufügen

Füge in `newRunCmd` zu den bestehenden `var`-Deklarationen hinzu:
```go
var customDockerfile string
```

### 2b — CollectFilePackages nach meta.Collect aufrufen

Finde den Block der `meta.Collect("")` aufruft und `meta.WriteXML` ausführt.
Füge zwischen `meta.Collect` und `meta.WriteXML` folgendes ein:

```go
			// Datei → Paket Zuordnung via dpkg -S
			filePackages, fpErr := meta.CollectFilePackages(outputFile)
			if fpErr != nil {
				log.Warnf("run: warning: CollectFilePackages fehlgeschlagen: %v", fpErr)
			} else {
				sysInfo.FilePackages = filePackages
				log.Infof("run: %d Dateien einem Paket zugeordnet", len(filePackages))
			}
```

### 2c — MetaFile und CustomDockerfile an dockerOpts übergeben

Finde den Block der `dockerOpts` befüllt. Füge hinzu:
```go
			dockerOpts.MetaFile = metaFile
			dockerOpts.CustomDockerfile = customDockerfile
```

Die Variable `metaFile` ist bereits in run.go definiert als:
```go
metaFile := programName + "_meta.xml"
```

### 2d — Flag registrieren

Füge bei den `cmd.Flags()`-Aufrufen am Ende von `newRunCmd` hinzu:
```go
cmd.Flags().StringVar(&customDockerfile, "dockerfile", "", "eigenes Dockerfile für den Docker-Build")
```

## Schritt 3 — Build prüfen

```bash
go build ./...
```

Erwartet: keine Fehler.

## Schritt 4 — Funktionstest (kein Docker nötig)

```bash
./bin/cb docker --help
```

Erwartet: Flags `--dockerfile`, `--meta`, `--base`, `--tag` werden angezeigt.

```bash
./bin/cb run --help
```

Erwartet: Flag `--dockerfile` wird angezeigt.

## Schritt 5 — Finaler Commit

```bash
git add cmd/cb/cmd/docker.go cmd/cb/cmd/run.go
git commit -m "feat(cmd): add --dockerfile and --meta flags to docker and run commands"
```
